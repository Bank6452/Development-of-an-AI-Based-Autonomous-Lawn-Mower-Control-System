...
void processCommand(String cmd) {
    ...
    if (header == "C" && first != -1) {
        int second = cmd.indexOf(',', first + 1);
        if (second == -1) return; // Bug #3: ป้องกัน second+1 เมื่อ second==-1
        ...
        if (second != -1 && third != -1) {
            ...
            int calcChk = (int)roundf(vL * 100) + (int)roundf(vR * 100); // Bug #8: ใช้ round แทน truncate
            if (calcChk == receivedChk) {
                if (emergencyMode) { stop(); return; }
                driveMotors(vL, vR);
                last_cmd_received = millis();
                // Bug #7: ไม่ reset data_stream_active ที่นี่
    ...
    // คำสั่งพิเศษ: R = Re-init IMU และ resume data stream (Bug #7: ย้าย resume มาที่นี่)
    if (header == "R") {
        initIMU();
        data_stream_active = true; // resume stream หลัง re-init
    }

    // Bug #6: G = สตาร์ทเครื่องยนต์ (Go)
    if (header == "G") {
        startEngine();
        Serial1.println("ENGINE: Starting...");
    }

    // Bug #6: X = ดับเครื่องยนต์ (eXtinguish)
    if (header == "X") {
        powerOff();
...
void setup() {
    ...
    last_cmd_received = millis();
    last_send = millis(); // ป้องกัน dt ผิดรอบแรก (Bug #2)
}
...
void loop() {
    ...
    // 3. อัปเดต IMU (เรียกครั้งเดียวต่อรอบ — ใช้ร่วมกันทั้ง engine starter และ data stream)
    //    ถ้า BNO055 ไม่ได้ต่อ → imu_ok=false → return ทันที ไม่บล็อก loop
    updateIMU();
    ...
    if (data_stream_active && (millis() - last_send >= 100)) { 
        ...
        // ส่งข้อมูล IMU BNO055 Heading (ใช้ค่าที่อ่านไว้แล้วจาก updateIMU ด้านบน)
