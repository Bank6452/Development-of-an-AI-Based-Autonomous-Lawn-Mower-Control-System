...
void initIMU() {
    ...
    // Bug #9: เรียก Wire.begin() ครั้งเดียว ป้องกัน I2C bus hang เมื่อ R command re-init
...
void updateIMU() {
    if (!imu_ok) return;  // ข้าม I2C ทั้งหมดถ้า sensor ไม่ได้ต่อ

    sensors_event_t event;
    bno.getEvent(&event);
    
    // ชดเชยค่า Yaw (ถ้ารถหันไปทิศเหนือแล้วเลขไม่ใช่ 0 ให้แก้ HEADING_OFFSET ใน main.cpp)
...
void displayCalibrationData() {
    ...
    uint8_t data[22] = {0}; // Bug #1: init เป็น 0 ป้องกัน garbage ถ้า Wire ส่งไม่ครบ
    Wire.beginTransmission(0x28);
    Wire.write(0x55);
